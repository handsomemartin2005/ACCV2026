...
from timm.models._registry import generate_default_cfgs, register_model_deprecations
...
__all__ = ['mobilevit_xxs', 'mobilevit_s', 'mobilevit_xs', 'mobilevitv2_050', 'mobilevitv2_100']
...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 17, 'input_size': (3, 640, 640), 'pool_size': (8, 8),
...
def mobilevit_xxs(pretrained=False, **kwargs) -> ByobNet:
        chs = []
        model = _create_mobilevit('mobilevit_xxs', pretrained=pretrained, features_only=True, **kwargs)
        out = model(torch.rand([1, 3, 224, 224]))
        for i in out:
            chs.append(i.shape[1])
        model.channel = chs
        return model
...
def mobilevit_xs(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevit_xs', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevit_s(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevit_s', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_050(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_050', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_075(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_075', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_100(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_100', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_125(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_125', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_150(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_150', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_175(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_175', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
...
def mobilevitv2_200(pretrained=False, **kwargs) -> ByobNet:
    chs = []
    model = _create_mobilevit('mobilevitv2_200', pretrained=pretrained, features_only=True, **kwargs)
    out = model(torch.rand([1, 3, 224, 224]))
    for i in out:
        chs.append(i.shape[1])
    model.channel = chs
