...
from ultralytics.nn.backbone.MobileViT import *
from ultralytics.nn.backbone.SwinTransformerV2 import *
from ultralytics.nn.backbone.DAMamba import mamba_adapters, sch_mamba_adapters
...
class BaseModel(nn.Module):
    """The BaseModel class serves as a base class for all the timm_models in the Ultralytics YOLO family."""
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        the provided list.
        分析给定输入时模型单层的计算时间和 FLOP。将结果附加到提供的列表中。
    ...
    def load(self, weights, verbose=True):
        ...
        """
        model = weights['model'] if isinstance(weights, dict) else weights  # torchvision timm_models are not dicts
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if hasattr(m, 'backbone'):
                ...
                for _ in range(5-len(x)):
                    x.insert(0, None)  #  len(x)=5

                for i_idx, i in enumerate(x):
                    # save = [2, 3, 7, 8, 12, 13, 16, 19, 22]
        ...
        out = head([y[j] for j in head.f], batch)  # head inference
        return out


class Ensemble(nn.ModuleList):
    """Ensemble of timm_models."""

    def __init__(self):
        """Initialize an ensemble of timm_models."""
        super().__init__()
...
def torch_safe_load(weight):
    ...
    try:
        with temporary_modules({
                ...
                'ultralytics.yolo.data': 'ultralytics.data'}):  # for legacy 8.0 Classify and Pose timm_models
            return torch.load(file, map_location='cpu', weights_only=False), file  # load

    except ModuleNotFoundError as e:  # e.name is missing module name
        if e.name == 'timm_models':
            raise TypeError(
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    """Loads an ensemble of timm_models weights=[a,b,c] or a single model weights=[a] or weights=a."""
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        """
        print(m)
        <function SwinTransformer_Tiny at 0x7bedaa470ee0>
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
            ...
            # f = [4, 16, 19, 22] -> ch[192, 256, 256, 256]
            # args = [17, [192, 256, 256, 256], 256, 300, 4, 8, 3]
            args.insert(1, [ch[x] for x in f])

        elif m in {GUIDE_RTDETRDecoder, SCH_GUIDE_RTDETRDecoder}:  # special case, channels arg must be passed in index 1
            # f = [4, 16, 19, 22] -> ch[192, 256, 256, 256]
            # args = [17, [256, 256, 256], 256, 300, 4, 8, 3]
            decoder_channel = []
            for x in f:
                decoder_channel.append(ch[x])
            ...
            if len(args) == 3:
                ...
                # len(args) = 2 -> [pretrained, img_size]
            elif len(args) == 2:
                m = timm.create_model(m, pretrained=args[0], img_size=args[1], features_only=True)
            elif len(args) == 1:
            ...
            c2 = m.feature_info.channels()
                   ...
                   mobilevit_xxs, mobilevit_s, mobilevit_xs, mobilevitv2_050, mobilevitv2_100,
                ...
                #    RevCol,
                   ...
                   # 直接调用整个backbone方法 -> SwinTransformer_Tiny, swinv2_tiny_window16_256
                   SwinTransformer_Tiny,
                   swinv2_tiny_window16_256,
                    mamba_adapters, sch_mamba_adapters,
...
def yaml_model_load(path):
    ...
    if path.stem in (f'yolov{d}{x}6' for x in 'nsmlx' for d in (5, 8)):
        ...
        LOGGER.warning(f'WARNING ⚠️ Ultralytics YOLO P6 timm_models now use -p6 suffix. Renaming {path.stem} to {new_stem}.')
