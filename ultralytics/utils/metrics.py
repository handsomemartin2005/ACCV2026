...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    ax.plot(px, py.mean(1), linewidth=3, color='blue', label='All Class %.3f' % ap[:, 0].mean())
...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric', on_plot=None):
    ...
    ax.plot(px, y, linewidth=3, color='blue', label=f'All Class')
...
class DetMetrics(SimpleClass):
    ...
    此类是一个实用程序类，用于计算对象检测模型的检测指标，例如精度、召回率和平均精度 (mAP)。
    ...
    Args:
        ...
        save_dir (路径)：保存输出图的目录路径。默认为当前目录。
        plot (bool)：指示是否为每个类绘制精确度-召回率曲线的标志。默认为 False。
        on_plot (func)：可选回调，用于在渲染图时传递图路径和数据。默认为 None。
        names (str 元组)：表示类名称的字符串元组。默认为空元组。
    ...
    def curves(self):
        """Returns a list of curves for accessing specific metrics curves."""
        # return ['Precision-Recall(M)', 'F1-Confidence(M)', 'Precision-Confidence(M)', 'Recall-Confidence(M)']
...
class SegmentMetrics(SimpleClass):
    ...
    @property
    def maps(self):
        """Returns mAP scores for object detection and semantic segmentation timm_models."""
        return self.box.maps + self.seg.maps

    @property
    def fitness(self):
        """Get the fitness score for both segmentation and bounding box timm_models."""
