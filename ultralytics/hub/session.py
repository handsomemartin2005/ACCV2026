...
class HUBTrainingSession:
    """
    HUB training session for Ultralytics HUB YOLO timm_models. Handles model initialization, heartbeats, and checkpointing.
    ...
    def __init__(self, url):
        ...
        # Parse input
        if url.startswith(f'{HUB_WEB_ROOT}/timm_models/'):
            url = url.split(f'{HUB_WEB_ROOT}/timm_models/')[-1]
        if [len(x) for x in url.split('_')] == [42, 20]:
            ...
            raise HUBModelError(f"model='{url}' not found. Check format is correct, i.e. "
                                f"model='{HUB_WEB_ROOT}/timm_models/MODEL_ID' and try again.")

        # Authorize
        auth = Auth(key)
        self.agent_id = None  # identifies which instance is communicating with server
        self.model_id = model_id
        self.model_url = f'{HUB_WEB_ROOT}/timm_models/{model_id}'
        self.api_url = f'{HUB_API_ROOT}/v1/timm_models/{model_id}'
    ...
    def _get_model(self):
        ...
        api_url = f'{HUB_API_ROOT}/v1/timm_models/{self.model_id}'
    ...
    def _start_heartbeat(self):
        ...
        while self.alive:
            r = smart_request('post',
                              f'{HUB_API_ROOT}/v1/agent/heartbeat/timm_models/{self.model_id}',
                              json={
                                  'agent': AGENT_NAME,
